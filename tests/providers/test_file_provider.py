...
def test_file_provider_parses_gpx(mock_create, mock_get, mock_sync_get, mock_sync_create, tmp_path):
...
@patch("fitler.providers.file.file_activity.FileActivity.get")
@patch("fitler.providers.file.file_activity.FileActivity.create") 
def test_file_provider_processes_multiple_files(mock_create, mock_get, mock_sync_get, mock_sync_create, tmp_path):
    ...
    provider = FileProvider(os.path.join(tmp_path, "*.gpx"), config={"home_timezone": "US/Eastern", "test_mode": True})
    activities = provider.pull_activities("2024-05")  # Match the date in our test data
